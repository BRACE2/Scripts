...
proc BuildOctColSectionElastic {ColSecTag  Dcol  nLbar  DLbar  sTbar} {
  ...
  # sTbar 				-> Spacing of transverse spiral reinforcement
	...
	##### PRINT LINE TO CHECK #####
	# puts "f3e=$f3e, fcc=$fcc, ecc=$ecc, ecu=$ecu, fcu=$fcu";
	...
	set Ets [expr $Ec/5.0];									# Tension softening stiffness *Check if causes numerical issues (Divide by constant to make flatter if needed)
	...
	uniaxialMaterial Elastic $IDconcCover $Ec;
	##### PRINT LINE TO CHECK #####
	# puts "Cover fpc = [expr -$fce], epsc0 = [expr -$Efact*$ec0]"
	# uniaxialMaterial Concrete02 $IDconcCore  [expr -$fcc] [expr -$ecc] [expr -$fcu] [expr -$ecu] $lambda $ftC $Ets;	# Core concrete (confined)
	uniaxialMaterial Elastic $IDconcCore $Ec;
	##### PRINT LINE TO CHECK #####
	...
	section Fiber $ColMatTag -GJ 1000000000000.0 {
		...
		set numSubdivJK 2;	# Determines # of fibers in the radial direction of the cover patch
	...
	uniaxialMaterial Elastic $IDTorsion [expr  0.2*$Gc*$Jcol];      # Define elastic torsional stiffness: Based on SDC-1.6 sec. 5.6.2 and sec. 1.1, fundamental period less than 0.7 sec, reduction is required *****CHECK FUNDAMENTAL PERIOD TO SEE IF REDUCTION STILL REQUIRED
	section Aggregator $ColSecTag $IDShear Vy $IDShear Vz $IDTorsion T -section $ColMatTag;
};

proc BuildWideOctColSectionElastic {ColSecTag  Dcol  Wcol  nLbar  nLbar2  DLbar  DLbar2  sTbar} {
  ...
  # Local Y axis is the horizontal axis.  Local Z axis is the vertical axis.  The longer width of the column is in the local Y direction.
	...
	section Fiber $ColMatTag -GJ 1000000000000.0 {
		...
		patch quad $IDconcCover 10 2 [expr -$spO] [expr -$Rcol] [expr $spO] [expr -$Rcol] [expr $spO] [expr -$Rcore] [expr -$spO] [expr -$Rcore]; # BOTTOM Outer rectangular COVER Patch, 10 fibers in y, 2 fibers in z
		patch quad $IDconcCover 10 2 [expr -$spO] [expr $Rcore] [expr $spO] [expr $Rcore] [expr $spO] [expr $Rcol] [expr -$spO] [expr $Rcol]; # TOP Outer rectangular COVER Patch, 10 fibers in y, 2 fibers in z
		set numSlices 8;	# Determines # of slices in each of the 8 sections of the octagon
		set numSubdivIJ 1;	# Determines # of fibers in the circumferential direction of the cover patch
		set numSubdivJK 2;	# Determines # of fibers in the radial direction of the cover patch
...
#--------------------------------------------------------------------------
proc BuildOctColSectionInel {ColSecTag  Dcol  nLbar  DLbar  sTbar} {
	...
	uniaxialMaterial Concrete02 $IDconcCore  [expr -$fcc] [expr -$ecc] [expr -$fcu] [expr -$ecu] $lambda $ftC $Ets;	# Core concrete (confined)
	# puts "uniaxialMaterial Concrete02 $IDconcCore  [expr -$fcc] [expr -$ecc] [expr -$fcu] [expr -$ecu] $lambda $ftC $Ets;	# Core concrete (confined)"
...
#--------------------------------------------------------------------------
proc BuildWideOctColSectionInel {ColSecTag  Dcol  Wcol  nLbar  nLbar2  DLbar  DLbar2  sTbar} {
  # OCTAGONAL WIDE COLUMN SECTION DEFINITION PROCEDURE - INELASTIC PROPERTIES WITH MODIFIED CRUSHING STRESS/STRAIN PAIR
...
#---------------------------------------------------------------------
proc BuildOctColSectionInelMod {ColSecTag  Dcol  nLbar  DLbar  sTbar} {
...
#---------------------------------------------------------------------
proc BuildWideOctColSectionInelMod {ColSecTag  Dcol  Wcol  nLbar  nLbar2  DLbar  DLbar2  sTbar} {
  # OCTAGONAL WIDE COLUMN SECTION DEFINITION PROCEDURE - INELASTIC PROPERTIES WITH MODIFIED CRUSHING STRESS/STRAIN PAIR
...
#---------------------------------------------------------------------
proc BuildCircColSectionInelMod {ColSecTag  Dcol  nLbar  DLbar  sTbar} {
	...
	##### PRINT LINE TO CHECK #####
	# puts "Building circular column section for column $ColSecTag"
	...
	# Column component dimensions
	set tcover 		[expr 2.0*$in];									# 2 inch cover width
	set Rcol		[expr $Dcol/2.0];								# Radius of column
	set Dcore		[expr $Dcol-2.0*$tcover];						# Diameter of core
	set Rcore		[expr $Dcore/2.0];								# Radius of core
	set Along		[expr $pi*$DLbar**2/4.0];						# Area of longitudinal reinforcement bar
	set DTbar		[expr 0.625*$in];								# Diameter of transverse spiral reinforcement bar (#5 Rebar)
	set Asp			[expr $pi*$DTbar**2/4.0];						# Area of transverse spiral reinforcement bar
	set Dtran		[expr $Dcore-$DTbar];							# Diameter of spiral of transverse spiral reinforcement
	...
	# Section Area Properties
	set Acol		[expr (2.0*$Dcol**2)/(1.0+sqrt(2.0))];			# Area of column section
	set Jcol		[expr $pi*$Rcol**4/2.0];	    				# Polar moment of inertia for column section
	set I3col		[expr $pi*$Rcol**4/4.0];	    				# Second moment of inertia, 1st transverse direction, for column section
	set I2col		[expr $pi*$Rcol**4/4.0];    					# Second moment of inertia, 2nd transverse direction, for column section
	...
	set numSubdivCirc 32; # Determines # of fibers in the circumferential direction, around the entire circumference, for the core fibers
	set ColMatTag [expr 10*$ColSecTag];    # Column material cross-section tag.  Set equal to 10*Column's fiber element tag.
	section Fiber $ColMatTag -GJ 1000000000000.0 {
		patch circ $IDconcCore $numSubdivCirc 10 0. 0. 0.0 [expr $Rcore] 0.000e+00 3.600e+02; # Core Patch, 10 radial fibers
		patch circ $IDconcCover $numSubdivCirc 2 0. 0. [expr $Rcore] [expr $Rcol] 0.000e+00 3.600e+02; # Cover Patch, 2 radial fibers
		layer circ $IDSteel $nLbar $Along 0. 0. $Rlong; # Longitudinal Bars
	}
	uniaxialMaterial Elastic $IDShear   [expr (9./10.)*$Gc*$Acol];	# Define elastic shear stiffness
	uniaxialMaterial Elastic $IDTorsion [expr  0.2*$Gc*$Jcol];      # Define elastic torsional stiffness: Based on SDC-1.6 sec. 5.6.2 and sec. 1.1, fundamental period less than 0.7 sec, reduction is required *****CHECK FUNDAMENTAL PERIOD TO SEE IF REDUCTION STILL REQUIRED
