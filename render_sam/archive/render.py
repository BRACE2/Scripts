...
def displaced_profile(
    ...
    v = linear_deformations(block_diag(*[Q]*reps)@displ, L)
    Lnew = L + v[0,0]
...
def plot_displ(frame:dict, res:dict, ax=None, axes=None):
    ...
    for el in frame["elems"].values():
        ...
        if "zero" not in el["type"].lower():
            ...
            vect = np.array(el["trsfm"]["vecInLocXZPlane"])[[axes[1], axes[0], axes[2]]]
...
class PlotlyPlotter(Plotter):
    ...
    def _get_displ(self,res:dict, name=None):
        ...
        for i,el in enumerate(frame["elems"].values()):
            ...
            if "zero" not in el["type"].lower():
                ...
                # vect = np.array(el["trsfm"]["vecInLocXZPlane"])
                vect = np.array(el["trsfm"]["vecInLocXZPlane"])[axes]
    ...
    def _get_nodes(self, displ=None, name=None):
        ...
        if not displ:
            ...
            x,y,z = xyz + np.array([displ.get(n,[0.0]*3)[:3] for n in self.model["nodes"].keys()]).T[self.axes]
...
def plot_plotly(model, axes=None, displ=None, opts=None):
    import plotly.graph_objects as go
    if opts is None:
        opts = Config()
