...
pss = v(:,1:pg)*(s(1:pg,1:pg)\uu(:,1:pg)');
...
qlin = v\Qb;  % Controllability Matrix used for the input-EMAC
...
for df = 1:nsizS
    fi((df-1)*m+1:df*m,1:n1) = CA_powers(:,:,df);
end
%
...
parfor df = 2:nsizS
    a = (df-1)*m+1;
    b = df*m;
    fi3(:,:,df) = block_3(df, m, CA_powers, dati, n1, r);
    %for nmf = 1:df-1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
toc
function fi = block_3(df, m, CA_powers, dati, n1, r)
    In1n1 = speye(n1,n1);
